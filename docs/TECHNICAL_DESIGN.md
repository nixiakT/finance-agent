...
- 模拟组合：`finance_build_paper_portfolio/finance_rebalance_paper_portfolio/finance_mark_paper_portfolio/finance_show_paper_portfolio`
...
- `finance_build_paper_portfolio`: 输入股票池和资金，例如 100 万，按行情、基本面、技术指标和数据源置信度评分，输出买入数量、目标仓位和风险提示。
...
账户写入 `.finance_agent/portfolio_default.json`，不提交到仓库。它只做纸面实验，不连接真实券商，不发送真实订单。
