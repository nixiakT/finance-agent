...
基础命令：
  ...
  /portfolio init 1000000 AAPL MSFT NVDA | /portfolio status | /portfolio mark | /portfolio sell AAPL all | /portfolio trades | /portfolio rebalance AAPL MSFT NVDA
    创建 100 万纸面投资账户，输出买入数量和仓位；之后可每日估值、模拟卖出、查看交易流水和再平衡。
...
Basics:
  ...
  /portfolio init 1000000 AAPL MSFT NVDA | /portfolio status | /portfolio mark | /portfolio sell AAPL all | /portfolio trades | /portfolio rebalance AAPL MSFT NVDA
    Create a paper account, mark to market daily, simulate sells, inspect trades, and rebalance. It never sends real orders.
  /learn-history AAPL 2y 20
