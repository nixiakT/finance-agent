...
class Memory:
    """Append-only Markdown memory stored outside one chat session."""

    def __init__(self, path: str | Path = DEFAULT_MEMORY_PATH):
        self.path = Path(path)

    def write(self, note: str) -> Path:
        """Persist one long-lived project/user convention."""
    ...
    def recall(self, query: str = "") -> str:  # noqa: ARG002 - future hook for filtered recall
        """Recall all project memory for the minimal Day7 implementation."""
...
class KVMemory:
    """Structured memory with update and forget semantics."""
