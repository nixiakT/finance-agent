...
- [x] 增加 100 万纸面模拟投资账户：按股票池评分、生成持仓/仓位/买入数量，并支持每日净值记录、模拟卖出、交易流水和再平衡。
...
- Paper Portfolio：`finance_build/rebalance/mark/show/sell/trades_paper_portfolio` 与 `/portfolio` 支持纸面建仓、模拟卖出、交易流水、每日估值和组合复盘。
